...
describe("useCSSOutput", () => {
  it("デフォルト値のみの場合は空の CSS ブロックを返す", () => {
    ...
    expect(result.current).toBe(".my-text {\n}");
  });
  ...
  it("CSS プロパティは kebab-case で出力される", () => {
    ...
    expect(result.current.cssOutput).toContain("font-size: 24px;");
    expect(result.current.cssOutput).toContain("font-weight: 700;");
    expect(result.current.cssOutput).toMatch(/^\.my-text \{/);
    expect(result.current.cssOutput).toMatch(/\}$/);
  });
});
