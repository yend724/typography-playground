...
本プロジェクトでは**関数型プログラミング**をベースとしたコーディングスタイルを採用する。

- **純粋関数を優先**: 副作用のない関数を基本とし、副作用は境界層（hooks, イベントハンドラ）に集約する
- **不変性**: データは不変として扱う。`Readonly<T>`, `readonly` 修飾子を活用し、直接の変更（mutation）を避ける
- **アロー関数**: 関数宣言ではなくアロー関数式で統一する（コンポーネント含む）
- **型安全なユニオン型**: `enum` は使わず、文字列リテラルのユニオン型で表現する
- **`type` を使用**: `interface` ではなく `type` で型を定義する（Props 型含む）
- **`any` 禁止**: `unknown` + 型ガードで安全に型を絞り込む
- **宣言的な記述**: 命令的なループよりも `map`, `filter`, `reduce` 等の高階関数を優先する
- **小さな関数への分割**: 1 つの関数は 1 つの責務を持ち、合成可能にする
...
- Props の型は同一ファイル内で `type` + `Readonly` として定義
